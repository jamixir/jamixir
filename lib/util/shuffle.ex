defmodule Shuffle do
  ...
  def shuffle(list, numeric_sequence)
    ...
    [element] ++ shuffle(new_list, Enum.slice(numeric_sequence, 1..-1))
  end

  # Formula (307) v0.3.5
  def shuffle(list, hash) when is_binary(hash) and bit_size(hash) == 256 do
    numeric_sequence = transform_hash_into_sequence(hash, length(list))
    shuffle(list, numeric_sequence)
  end
  ...
  defp transform_hash_into_sequence(hash, sequence_length) do
    numeric_sequence =
      Enum.reduce(0..(sequence_length - 1), [], fn i, acc ->
        ...
        new_hash = Util.Hash.blake2b_n(hash <> encoded_chunk, 36)
        encoded_numeric_position = :binary.part(new_hash, 4 * i, 4)
