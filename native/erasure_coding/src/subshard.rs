	...
	pub fn new() -> Result<Self, Error> {
		Ok(Self {
			...
			encoder: reed_solomon::ReedSolomonEncoder::new(
				...
				BATCH_SHARD_SIZE,
			).map_err(|_| Error::Atom("ReedSolomonEncoding Error"))?,
		})
	}
	...
	pub fn construct_chunks(
		...
		for segments in segments.chunks(SEGMENTS_PER_SUBSHARD_BATCH_OPTIMAL) {
			...
			if self.last_shard_size != s {
				self.encoder.reset(N_SHARDS, N_REDUNDANCY * N_SHARDS, s).map_err(|e| Error::Atom("ReedSolomonResetError"))?;
			...
			for shard_a in 0..N_SHARDS {
				...
				self.encoder.add_original_shard(&shard[..s]).map_err(|e| Error::Atom("AddoriginalShardError"))?;
			}

			let enco_res = self.encoder.encode().map_err(|e| Error::Atom("ReedSolomonEncodeError"))?;
	...
	pub fn new() -> Result<Self, Error> {
		...
		Ok(Self {
			...
			decoder: reed_solomon::ReedSolomonDecoder::new(
				...
				N_REDUNDANCY * N_SHARDS,
				BATCH_SHARD_SIZE,
			).map_err(|_| Error::Atom("ReedSolomonNewError"))?,
		...
		I: Iterator<Item = (u8, ChunkIndex, &'a SubShard)>,
	{
		...
		for segments in segment_batches {
			...
			if s != self.last_shard_size {
				self.decoder.reset(N_SHARDS, N_REDUNDANCY * N_SHARDS, s).map_err(|e| Error::Atom("ReedSolomonResetError"))?;
				self.last_shard_size = s;
			}
			let mut nb_chunk = 0;
			...
			for (chunk_ix, chunks) in ori.iter().enumerate() {
				...
				if nb > 0 {
					if chunk_ix < N_SHARDS {
						self.decoder
							.add_original_shard(chunk_ix, &self.shards_ori[chunk_ix][..s]).map_err(|e| Error::Atom("add_original_shardError"))?;
						ori_map.insert(chunk_ix, &self.shards_ori[chunk_ix][..]);
					} else {
						self.decoder.add_recovery_shard(chunk_ix - N_SHARDS, &shard_buff[..s]).map_err(|e| Error::Atom("add_recovery_shardError"))?;
					}
					nb_chunk += 1;
			...
			let ori_ret = self.decoder.decode().map_err(|e| Error::Atom("ReedSolomonDecodeError"))?;
			nb_decode += 1;
