...
fn encode_str(d_hex: &str, V: usize, C: usize) -> Result<(), Box<dyn Error>> {
    let d_bytes = hex::decode(d_hex.trim_start_matches("0x"))?;
    let d_bytes_len = d_bytes.len();
    let result = do_encode(d_bytes, C)?;

    let mut shards_json = Vec::new();
    for shard in &result {
        shards_json.push(format!("0x{}", hex::encode(shard)));
    }
    ...
    println!(
        "✅ encode SUCCESS {} bytes into {} shards ({} original, {} recovery) → {}",
        d_bytes_len,
        V,
        C,
        V - C,
...
#[rustler::nif]
fn encode<'a>(env: rustler::Env<'a>, data_binary: Binary<'a>, c: usize) -> NifResult<Vec<Binary<'a>>> {
        ...
        match result {
            Ok(decoded) => {
                ...
                if trimmed != orig_bytes {
                    return Err(format!(
                        "Trial {} failed: reconstructed segment does not match expected",
                        trial
                    )
                    .into());
                } else {
...
mod tests {
    ...
    fn test_encode_decode() -> Result<(), Box<dyn Error>> {
        ...
        encode_str(b10, 1023, 342)?;
        encode_str(b10, 6, 2)?;
        encode_str(b272, 6, 2)?;
        encode_str(s4104, 6, 2)?;
        encode_str(b272, 1023, 342)?;
        encode_str(s4104, 1023, 342)?;
