...
use rustler::{Error, Encoder, Env, NifResult, Term};
...
pub const fn recovery_threshold(n_chunks: u16) -> Result<u16, Error> {
    if n_chunks > MAX_CHUNKS {
        return Err(Error::Atom("TooManyTotalChunks"));
    }
    if n_chunks == 0 {
        return Err(Error::Atom("NotEnoughTotalChunks"));
    }
...
pub fn reconstruct_from_systematic<'a>(
    ...
    if systematic_len < k {
        return Err(Error::Atom("NotEnoughChunks"));
    }
    ...
    for chunk in systematic_chunks.by_ref() {
        ...
        if shard_len == 0 {
            ...
            if shard_len % SHARD_ALIGNMENT != 0 && nb != k {
                return Err(Error::Atom("UnalignedChunk"));
            }
        ...
        if chunk.len() != shard_len {
            return Err(Error::Atom("NonUniformChunks"));
        }
...
#[rustler::nif]
pub fn encode_native(n_chunks: u16, data: Vec<u8>) -> NifResult<Vec<Vec<u8>>> {
    construct_chunks(n_chunks, data)
}
...
// #[rustler::nif]
pub fn construct_chunks(n_chunks: u16, data: Vec<u8>) -> NifResult<Vec<Vec<u8>>> {
    if data.is_empty() {
        return Err(Error::Atom("BadPayload"));
    }
    if n_chunks == 1 {
        return Ok(vec![data]);
    }
    let systematic = systematic_recovery_threshold(n_chunks)?;
    let original_data = make_original_shards(systematic, data.as_slice());
    ...
    let recovery = reed_solomon::encode(original_count, recovery_count, original_iter).map_err(|_| Error::Atom("ReedSolomonErrorEncoding"))?;
    ...
    I: IntoIterator<Item = (ChunkIndex, Vec<u8>)>,
{
    if n_chunks == 1 {
        return chunks
            ...
            .next()
            .map(|(_, v)| v)
            .ok_or(Error::Atom("NotEnoughChunks"));
    }
    ...
    let mut recovered =
        reed_solomon::decode(original_count, recovery_count, original_iter, recovery).map_err(|e| Error::Atom("ReedSolomonErrorDecoding"))?;
...
mod tests {
    ...
    The GP calls for 2-byte chunks and 4104-byte pieces in Appendix H.
    ...
    Below is a test_reconstruct_from_json you can add around here:
    ...
    The test uses the last 342 2-byte chunks to reconstruct the first 684 2-byte chunks (684+342=1026),
    and checks that the first 342 chunk match the original data.
    */
    ...
    fn test_reconstruct_from_json() {
        ...
        // Define the path to the test data file relative to the current file's directory
        let test_data_path = current_dir.join("test_data.json");
        ...
        let s = &mut subshards
            .iter()
            .map(|(a, b, c)| (*a, *b, c.as_slice().try_into().unwrap()));

        let (reconstructed_segments, _nb_decode) = decoder.reconstruct(s).unwrap();
    ...
    fn round_trip_systematic_works() {
        fn property(available_data: ArbitraryAvailableData, n_chunks: u16) {
            ...
            let chunks = construct_chunks(n_chunks, available_data.clone().0).unwrap();
    ...
    fn round_trip_works() {
        fn property(available_data: ArbitraryAvailableData, n_chunks: u16) {
            ...
            let chunks = construct_chunks(n_chunks, available_data.clone().0.into()).unwrap();
            ...
            assert_eq!(reconstructed, available_data.0.as_slice());
        }
    ...
    fn proof_verification_works() {
        fn property(data: SmallAvailableData, n_chunks: u16) {
            ...
            let chunks = construct_chunks(n_chunks, data.0.into()).unwrap();
