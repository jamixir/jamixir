defmodule Jamixir.NodeTest do
  ...
  describe "validator index and assigned shards  " do
    ...
    test "returns nil no state found" do
      assert my_validator_index(nil) == nil
    end

    test "returns nil when no index found", %{state: state} do
      assert my_validator_index(state) == nil
    end

    test "returns correct index", %{state: state} do
      state = assign_me_to_index(state, 3)
      assert my_validator_index(state) == 3
    end

    test "returns nil when not in validators list shard index", %{state: state} do
      assert my_assigned_shard_index(state, 2) == nil
    end

    # i = (cR + v) mod V
    test "returns correct shard index", %{state: state} do
      state = assign_me_to_index(state, 2)
      # i = (1 * 2 + 2) mod 6 = 4
      assert my_assigned_shard_index(state, 1) == 4
      # i = (0 * 2 + 2) mod 6 = 2
      assert my_assigned_shard_index(state, 0) == 2
    end

    test "returns correct shard index overflow division", %{state: state} do
      state = assign_me_to_index(state, 5)
      # i = (1 * 2 + 5) mod 6 = 1
      assert my_assigned_shard_index(state, 1) == 1
    end
  end
  ...
  defp assign_me_to_index(state, index) do
    ...
    put_in(state.curr_validators, new_curr)
  end
end
