defmodule System.StateTransition.AuthorizerPoolTest do
  ...
  test "removing no authorizer works" do
    # Mock guarantees
    guarantees = []

    # Mock posterior authorizer queue
    posterior_authorizer_queue = Enum.map(1..Constants.core_count(), fn i ->
      Enum.map(1..Constants.max_authorization_queue_items(), fn j -> "queue#{i}_#{j}" end)
    end)

    # Mock authorizer pools
    authorizer_pools = Enum.map(1..Constants.core_count(), fn i ->
    ...
    # Call the function
    result = State.posterior_authorizer_pool(
      guarantees,
    ...
    expected_result = Enum.map(1..Constants.core_count(), fn i ->
      ["auth#{i}_2", "auth#{i}_3", "auth#{i}_4", "auth#{i}_5", "auth#{i}_6", "auth#{i}_7", "auth#{i}_8", "queue#{i}_3"]
    end)
  ...
  test "removing third authorizer from each core works" do
    ...
    guarantees = Enum.map(1..Constants.core_count(), fn i ->
    ...
    expected_result = Enum.map(1..Constants.core_count(), fn i ->
      ["auth#{i}_1", "auth#{i}_2", "auth#{i}_4", "auth#{i}_5", "auth#{i}_6", "auth#{i}_7", "auth#{i}_8", "queue#{i}_3"]
    end)
